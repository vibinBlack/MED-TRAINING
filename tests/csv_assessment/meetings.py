...
def schedule_meeting():
    ...
    for i in csv_dict.keys():
        ...
        for j in csv_dict[i]['scheduled meetings']:
            ...
            csv_dict[i]['scheduled_in'].append(datetime(2000, 1, 1, hour=scheduled_in[0],
                                                        minute=scheduled_in[1]))
            csv_dict[i]['scheduled_out'].append(datetime(2000, 1, 1, hour=scheduled_out[0],
                                                         minute=scheduled_out[1]))
    ...
    for i in range(len(csv_dict[meeting_bw[0]]['scheduled_out'])):
        for j in range(len(csv_dict[meeting_bw[0]]['scheduled_out'])):
            ...
            if csv_dict[meeting_bw[0]]['scheduled_in'][j] < meeting_start <= \
                    csv_dict[meeting_bw[0]]['scheduled_out'][j]:
                meeting_start = csv_dict[meeting_bw[0]]['scheduled_out'][j]
                meeting_end = meeting_start + timedelta(minutes=duration)
                if csv_dict[meeting_bw[0]]['scheduled_in'][j] < \
                        meeting_end <= csv_dict[meeting_bw[0]]['scheduled_out'][j]:
                    meeting_start = csv_dict[meeting_bw[0]]['scheduled_out'][j]
            meeting_end = meeting_start + timedelta(minutes=duration)
            if csv_dict[meeting_bw[0]]['scheduled_in'][j] < meeting_end \
                    <= csv_dict[meeting_bw[0]]['scheduled_out'][j]:
                meeting_start = csv_dict[meeting_bw[0]]['scheduled_out'][j]
            ...
            if meeting_end - meeting_start > csv_dict[meeting_bw[0]]['scheduled_out'][j] \
                ...
                if meeting_start < csv_dict[meeting_bw[0]]['scheduled_in'][j] <= meeting_end:
                    meeting_start = csv_dict[meeting_bw[0]]['scheduled_out'][j]
                    meeting_end = meeting_start + timedelta(minutes=duration)
                    if meeting_start < csv_dict[meeting_bw[0]]['scheduled_out'][j] <= meeting_end:
                        meeting_start = csv_dict[meeting_bw[0]]['scheduled_out'][j]
