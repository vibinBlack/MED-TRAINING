...
def test_dictionary_length():
    '''Testing dictionary length'''
    word_to_digit = { "one" : 1, "two" : 2}
    assert len(word_to_digit) == 2 #note that a key value pair is treated as one item

def test_dictionary_is_indexed_by_key():
    '''testing dictionary can be indexed'''
    word_to_digit = { "one" : 1, "two" : 2}
...
def test_dictionary_is_mutable():
    ...
    #  When we want to make sure that items are returned in the order they were inserted,
    #  we can use OrderedDict.


def test_dictionary_is_unordered():
    '''testing dictionary is unordered'''
    dict1 = { 'one': 1, 'two': 2 }
    dict2 = { 'two': 2, 'one': 1}
...
def test_valid_dictionary_keys():
    ...
    try:
        ...
        print (exp)  #observe the error message.
        assert True
