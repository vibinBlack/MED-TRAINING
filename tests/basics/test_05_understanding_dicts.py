...
def test_dictionary_keys_and_values():
    ...
    keys = list(word_to_digit.keys()) #Dictionaries are orderless,So Keys are converted to list for sorting purpose
    #sort to get a deterministic order
    keys.sort()
    assert ['one', 'two'] == keys
    values = list(word_to_digit.values()) #Dictionaries are orderless,So values are converted to list for sorting purpose
    values.sort()
    assert [1, 2] == values
...
-Dictionaries Cannot be sorted directly, but can be converted to other types, such as lists and tuples and then sorted
-
-
"""
