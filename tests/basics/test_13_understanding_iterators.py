...
def test_enumerate_iter():
    ...
    try:
        ...
        assert  True == next(list_iter) #note what happens when items are finished.  => 'StopIteration' is raised
        #                                                                             =>  as it has no items left
...
def test_convert():
    ...
    try:
        ...
        assert True       # sequence item 0: expected str instance, int found
...
def test_find_builtins_that_work_on_iterables():
    funcs = [1,2,3,4]
    assert 4 == len(funcs)   # iter, StopIteration, next(iterable_object)
