...
def test_enumerate_iter():
    '''testing enumerate type iterable'''
    list_iter = iter(["one", "two", "three"])
    try:
        assert list_iter.__next__() == "one"
        assert list_iter.__next__() == "two"
        assert list_iter.__next__() == "three"
        assert list_iter.__next__() is None  #note what happens when items are finished.
...
def test_functions_that_work_on_iterables():
    '''testing functions that work on iterables'''
...
def test_find_builtins_that_work_on_iterables():
    '''builtin functions that work on iterables'''
    funcs = [all, any, filter, tuple, list, dict, set, map, max, min, sorted, sum, zip]
    assert len(funcs) == 13


THREE_THINGS_I_LEARNT = """
-iterators are used to loop through a object
-enumerate iter
-
"""
