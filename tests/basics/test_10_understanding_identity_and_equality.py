...
def test_identity_equality_small_numbers():
    ...
    9987148
    >>> id(b)
    9987148                      same reference for a and b  [-5 to 256]
    >>> a = 257
    >>> b = 257
    >>> id(a)
    11662816
    >>> id(b)
    11662828
    The current implementation keeps an array of integer objects for all integers between -5 and 256, when you create an int in that 
    range you actually just get back a reference to the existing object. So it should be possible to change the value of 1. I suspect 
    the behaviour of Python in this case is undefined.
    """
    a = b = 10
    assert True == (a is b)
    assert True == ( a == b)
